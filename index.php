<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Quadacts</title>
    <link rel="stylesheet" href="style.css" />
    <link
      href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css"
      rel="stylesheet"
      integrity="sha384-rbsA2VBKQhggwzxH7pPCaAqO46MgnOM80zW1RWuH61DGLwZJEdK2Kadq2F9CUG65"
      crossorigin="anonymous"
    />
    <link
      href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
      href="https://cdn.jsdelivr.net/npm/intl-tel-input@25.3.1/build/css/intlTelInput.css"
    />
    <script src="https://cdn.jsdelivr.net/npm/intl-tel-input@25.3.1/build/js/intlTelInput.min.js"></script>
  </head>
  <script type="text/javascript">
    var Tawk_API = Tawk_API || {},
      Tawk_LoadStart = new Date();
    (function () {
      var s1 = document.createElement("script"),
        s0 = document.getElementsByTagName("script")[0];
      s1.async = true;
      s1.src = "https://embed.tawk.to/67f9552343ce3e190eb807f5/1ioiv94sd";
      s1.charset = "UTF-8";
      s1.setAttribute("crossorigin", "*");
      s0.parentNode.insertBefore(s1, s0);
    })();
  </script>
  <body>
    <div class="top">
      <h6 class="fw-bold text-white text-center m-0">
        100% In-House Developers - No Freelancers, Only Experts!
      </h6>
    </div>
    <nav class="navbar navbar-expand-lg bg-light sticky-top">
      <div class="container justify-content-between align-items-center">
        <a class="navbar-brand" href="#">
          <img src="images/Quadacts-logo.webp" class="logo" alt="" />
        </a>
        <!-- <button
          class="navbar-toggler"
          type="button"
          data-bs-toggle="collapse"
          data-bs-target="#navbarNavAltMarkup"
          aria-controls="navbarNavAltMarkup"
          aria-expanded="false"
          aria-label="Toggle navigation"
        >
          <span class="navbar-toggler-icon"></span>
        </button> -->
        <!-- <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
          <div class="navbar-nav">
            <a class="nav-link active" aria-current="page" href="#">Home</a>
            <a class="nav-link" href="#">Features</a>
            <a class="nav-link" href="#">Pricing</a>
            <a class="nav-link disabled">Disabled</a>
          </div>
        </div> -->
        <div>
          <h4 class="m-0 nav-text">
            Risk-Free 1-Week Trial - Only Pay When Satisfied!
          </h4>
        </div>
        <div class="banner-btn">
          <button
            class="btn btn-theme"
            data-bs-toggle="modal"
            data-bs-target="#myModal"
          >
            Get Started Today
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              viewBox="0 0 174 174"
              fill="none"
              class="ms-2"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M87 174C135.049 174 174 135.049 174 87C174 38.9512 135.049 0 87 0C38.9512 0 0 38.9512 0 87C0 135.049 38.9512 174 87 174ZM37 65V108.5H85.5V130L141 88L85.5 44V65H37Z"
                fill="white"
              />
            </svg>
          </button>
        </div>
      </div>
    </nav>
    <div class="banner">
      <div class="container">
        <div class="row align-center">
          <div class="col-lg-6">
            <div class="content">
              <h4>Your Vision, Our Code - Delivered Fast!</h4>
              <h1>
                Software <strong>Developers</strong><br />
                on Contract in <span class="text-dark">1 Day</span>
              </h1>
              <!-- <ul> -->
              <p>Powerful & Scalable Tech Teams, On Demand</p>
              <p>Hire Top-Tier Developers in Just 1 Day!</p>
              <p>
                Flexible, Scalable, and Realiable - Hire Expert Developers
                Instantly!
              </p>
              <!-- </ul> -->
              <!-- <p class="mt-3">
                Transforming the startups and enterprises by providing
                end-to-end solutions in Software, Web, Mobile app, Artificial
                Intelligence, and Custom Development.
              </p> -->
              <div class="offer-content">
                <div>
                  <h2>1 week</h2>
                  <p>Risk-free trail</p>
                </div>
                <div>
                  <img src="images/divider.png" alt="" />
                </div>
                <div>
                  <h2>100%</h2>
                  <p>in-House Developers</p>
                </div>
              </div>
              <div class="banner-btn mt-3">
                <button class="btn btn-theme">
                  Hire Developer
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                    viewBox="0 0 174 174"
                    fill="none"
                    class="ms-2"
                  >
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
                      d="M87 174C135.049 174 174 135.049 174 87C174 38.9512 135.049 0 87 0C38.9512 0 0 38.9512 0 87C0 135.049 38.9512 174 87 174ZM37 65V108.5H85.5V130L141 88L85.5 44V65H37Z"
                      fill="white"
                    />
                  </svg>
                </button>
              </div>
            </div>
          </div>
          <div class="col-lg-6">
            <div class="thumb">
              <img
                src="images/home-banner-image.webp"
                class="home-page-banner-image"
                alt=""
              />
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="services">
      <div class="our-services-section container">
        <h2 class="before-content">Our Services</h2>
        <h4 class="sub-heading">
          QuadActs Payroll Developers | No Freelancers | No Sub Contracting
        </h4>
        <div class="section-content">
          <div class="section-item-wrapper">
            <div class="section-item first-section">
              <div class="section-title">
                <h3 class="text-center">Dedicated Developers</h3>
                <p class="text-center">
                  QuadActs employees will be dedicated solely to your project
                  for the entire duration.
                </p>
                <div class="d-flex justify-content-center">
                  <img
                    src="images/under_line_blue.png"
                    class="line-image"
                    alt="underline image"
                  />
                </div>
              </div>
              <ul class="list-items">
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text"
                    >Access to Senior Developers</span
                  >
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text">In-House Training</span>
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Tracking Software</span>
                  </span>
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text">Accreditation Certificate</span>
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text">Up to 45 Hours per Week</span>
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text"
                    >Senior Developer Work Review</span
                  >
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text">Rapid Replacement</span>
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text">Immediate Onboarding</span>
                </li>
                <li class="icon-list-item">
                  <span class="icon-list-icon">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="17"
                      height="15"
                      viewBox="0 0 17 15"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                        fill="#00E9FF"
                      ></path>
                    </svg>
                  </span>
                  <span class="icon-list-text"
                    >1-Month Release Notice for Resources
                  </span>
                </li>
              </ul>
              <div class="hire-now-button">
                <a href="#" role="button">
                  <span style="margin-right: 10px">Hire Developer</span>
                  <span>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512 512"
                      height="14"
                      width="14"
                    >
                      <path
                        d="M502.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-128-128c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L402.7 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l370.7 0-73.4 73.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l128-128z"
                        fill="#489351"
                      />
                    </svg>
                  </span>
                </a>
              </div>
            </div>
            <div class="section-item-wrapper">
              <div class="section-item second-section">
                <div class="tag-img">
                  <img src="images/popular_img.png" alt="popular-tag image" />
                </div>
                <div>
                  <h3 class="text-center text-black">
                    Offshore Managed Team Solutions
                  </h3>
                  <p class="text-white text-center">
                    Companies can now establish their entire tech team with
                    QuadActs
                  </p>
                </div>
                <div class="gradient-background">
                  <ul class="text-center m-0">
                    <li>Minimum 10 member team</li>
                    <li>minimum 1 year contract</li>
                  </ul>
                </div>
                <ul class="list-items">
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Define Your Work Culture</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Customized Policies</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >Requirement-Based Market Hiring</span
                    >
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >Training for Specific Skillsets</span
                    >
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >Direct Credit of Gifts and Bonuses to Resources</span
                    >
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Flexible Working Hours</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >Comprehensive Dedicated Developer Package</span
                    >
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >Transparent Pricing (Detailed Cost Breakdown
                      Provided)</span
                    >
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Dedicated HR Manager</span>
                  </li>
                </ul>
                <div class="hire-now-button">
                  <a
                    href="https://quadacts.com/contact-us/"
                    class=""
                    role="button"
                  >
                    <span style="margin-right: 10px">Hire Team</span>
                    <span>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 512 512"
                        height="14"
                        width="14"
                      >
                        <path
                          d="M502.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-128-128c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L402.7 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l370.7 0-73.4 73.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l128-128z"
                          fill="#489351"
                        />
                      </svg>
                    </span>
                  </a>
                </div>
              </div>
            </div>
            <div class="section-item-wrapper">
              <div class="section-item third-section">
                <div class="section-title">
                  <h3 class="text-center">Fixed-Price Project Management</h3>
                  <p class="text-center">
                    Turn your idea into software at a fixed cost.
                  </p>
                  <div class="d-flex justify-content-center">
                    <img
                      src="images/under_line_blue.png"
                      alt="underline image"
                      class="line-image"
                    />
                  </div>
                </div>
                <ul class="list-items">
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >Submit Requirement Document</span
                    >
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Request Quotation</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Single Point of Contact</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="15"
                        viewBox="0 0 17 15"
                        fill="none"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M0 7.22668L3.1377 7.18568C4.34961 7.87582 5.43424 8.76959 6.36808 9.89978C8.78361 5.94342 11.5588 2.7018 14.6094 0H17C12.7348 4.68067 9.25399 9.69205 6.47046 15C4.98185 11.8472 2.89974 9.19324 0 7.22668Z"
                          fill="#00E9FF"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Milestone-Based Reports</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        width="15"
                        height="15"
                        viewBox="0 0 15 15"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M8.90988 9.70575C8.43874 9.15796 7.94464 8.60162 7.42703 8.0508C5.73007 9.68537 4.15863 11.5259 3.0214 13.6067C1.62632 16.1592 -0.432388 14.0705 0.501674 12.2205C1.25253 10.5328 3.18328 8.19126 5.50394 6.15955C4.22589 5.01053 2.82507 3.97443 1.29081 3.21072C-0.95438 2.09282 0.760893 0.21947 2.38511 0.962531C3.75339 1.5084 5.64148 2.91382 7.36878 4.66186C9.09445 3.39882 10.9112 2.40899 12.5491 2.02149C12.9045 1.93721 13.9422 1.70945 14.1812 2.05013C14.2712 2.17875 14.1722 2.37181 13.9876 2.5814C13.359 3.29637 11.6694 4.42996 10.8507 5.08269C10.2598 5.55392 9.65992 6.04636 9.06328 6.56193C9.69765 7.35539 10.2648 8.17694 10.7168 8.99023C11.1097 9.69694 11.4154 10.3976 11.6032 11.0685C11.6891 11.375 11.9245 12.2704 11.641 12.4913C11.5341 12.5747 11.3645 12.4957 11.1775 12.3431C10.5393 11.8228 9.5016 10.394 8.90988 9.70575Z"
                          fill="#EB0100"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Developer Access</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        width="15"
                        height="15"
                        viewBox="0 0 15 15"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M8.90988 9.70575C8.43874 9.15796 7.94464 8.60162 7.42703 8.0508C5.73007 9.68537 4.15863 11.5259 3.0214 13.6067C1.62632 16.1592 -0.432388 14.0705 0.501674 12.2205C1.25253 10.5328 3.18328 8.19126 5.50394 6.15955C4.22589 5.01053 2.82507 3.97443 1.29081 3.21072C-0.95438 2.09282 0.760893 0.21947 2.38511 0.962531C3.75339 1.5084 5.64148 2.91382 7.36878 4.66186C9.09445 3.39882 10.9112 2.40899 12.5491 2.02149C12.9045 1.93721 13.9422 1.70945 14.1812 2.05013C14.2712 2.17875 14.1722 2.37181 13.9876 2.5814C13.359 3.29637 11.6694 4.42996 10.8507 5.08269C10.2598 5.55392 9.65992 6.04636 9.06328 6.56193C9.69765 7.35539 10.2648 8.17694 10.7168 8.99023C11.1097 9.69694 11.4154 10.3976 11.6032 11.0685C11.6891 11.375 11.9245 12.2704 11.641 12.4913C11.5341 12.5747 11.3645 12.4957 11.1775 12.3431C10.5393 11.8228 9.5016 10.394 8.90988 9.70575Z"
                          fill="#EB0100"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text">Dedicated Developer Team</span>
                  </li>
                  <li class="icon-list-item">
                    <span class="icon-list-icon">
                      <svg
                        width="15"
                        height="15"
                        viewBox="0 0 15 15"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M8.90988 9.70575C8.43874 9.15796 7.94464 8.60162 7.42703 8.0508C5.73007 9.68537 4.15863 11.5259 3.0214 13.6067C1.62632 16.1592 -0.432388 14.0705 0.501674 12.2205C1.25253 10.5328 3.18328 8.19126 5.50394 6.15955C4.22589 5.01053 2.82507 3.97443 1.29081 3.21072C-0.95438 2.09282 0.760893 0.21947 2.38511 0.962531C3.75339 1.5084 5.64148 2.91382 7.36878 4.66186C9.09445 3.39882 10.9112 2.40899 12.5491 2.02149C12.9045 1.93721 13.9422 1.70945 14.1812 2.05013C14.2712 2.17875 14.1722 2.37181 13.9876 2.5814C13.359 3.29637 11.6694 4.42996 10.8507 5.08269C10.2598 5.55392 9.65992 6.04636 9.06328 6.56193C9.69765 7.35539 10.2648 8.17694 10.7168 8.99023C11.1097 9.69694 11.4154 10.3976 11.6032 11.0685C11.6891 11.375 11.9245 12.2704 11.641 12.4913C11.5341 12.5747 11.3645 12.4957 11.1775 12.3431C10.5393 11.8228 9.5016 10.394 8.90988 9.70575Z"
                          fill="#EB0100"
                        ></path>
                      </svg>
                    </span>
                    <span class="icon-list-text"
                      >On-the-Go Requirement Changes</span
                    >
                  </li>
                </ul>
                <div class="hire-now-button">
                  <a
                    href="https://quadacts.com/contact-us/"
                    class=""
                    role="button"
                  >
                    <span style="margin-right: 10px">Get Quote</span>
                    <span>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 512 512"
                        height="14"
                        width="14"
                      >
                        <path
                          d="M502.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-128-128c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L402.7 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l370.7 0-73.4 73.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l128-128z"
                          fill="#489351"
                        />
                      </svg>
                    </span>
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="framework-section">
      <div class="techies-section container">
        <div class="section-content-area">
          <h2 class="before-content">
            Focused On Being The Best In JAVASCRIPT Framework
          </h2>
          <h4 class="sub-heading">Techies In Practice</h4>
          <div class="section-content">
            <div class="section-item-wrapper second-section">
              <div class="section-item">
                <div>
                  <div class="section-title">
                    <h4>DEVOPS</h4>
                  </div>
                  <div class="techies-images-wrapper">
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/DockerLogo.png" alt="docker icon" />
                        <p>Docker</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/AzureLogo.png" alt="azure icon" />
                        <p>Azure</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/AwsLogo.png" alt="aws icon" />
                        <p>AWS</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/JenkinsLogo.png" alt="jenkins icon" />
                        <p>Jenkins</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/AnsibleLogo.png" alt="ansible icon" />
                        <p>Ansible</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img
                          src="images/KubernetesLogo.png"
                          alt="kubernetes icon"
                        />
                        <p>Kubernetes</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img
                          src="images/GoogleCloud.png"
                          alt="google_cloude icon"
                        />
                        <p>Google Cloud</p>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="section-item-wrapper first-section">
              <div class="section-item">
                <div>
                  <div class="section-title">
                    <h4>WEB FRONTEND</h4>
                  </div>
                  <div class="techies-images-wrapper">
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/AngularLogo.png" alt="anguler icon" />
                        <p>Anguler</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/HtmlLogo.png" alt="html icon" />
                        <p>HTML</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/CssLogo.png" alt="css icon" />
                        <p>CSS</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img
                          src="images/BootstrapLogo.png"
                          alt="bootstrap icon"
                        />
                        <p>Bootstrap</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/ReactLogo.png" alt="react js icon" />
                        <p>React JS</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img
                          src="images/TypeScript.png"
                          alt="type_script icon"
                        />
                        <p>Type Script</p>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section-item">
                <div>
                  <div class="section-title">
                    <h4>BACKEND</h4>
                  </div>
                  <div class="techies-images-wrapper">
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/Nextjs.png" alt="nest_js icon" />
                        <p>NestJS</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/Nodejs.png" alt="nodejs icon" />
                        <p>NodeJS</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/LaravelLogo.png" alt="laravel icon" />
                        <p>Laravel</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/SocketLogo.png" alt="Socket_io icon" />
                        <p>Socket.io</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img
                          src="images/ExpressLogo.png"
                          alt="express_js icon"
                        />
                        <p>Express.jS</p>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section-item">
                <div>
                  <div class="section-title">
                    <h4>MOBILE APP</h4>
                  </div>
                  <div class="techies-images-wrapper">
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/FlutterLogo.png" alt="flutter icon" />
                        <p>Flutter</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/ReactNative.png" alt="reactjs icon" />
                        <p>React Native</p>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
              <div class="section-item">
                <div>
                  <div class="section-title">
                    <h4>DATABASE</h4>
                  </div>
                  <div class="techies-images-wrapper">
                    <div class="single-tech">
                      <a href="#">
                        <img
                          src="images/PostgreSQL.png"
                          alt="postgre_sql icon"
                        />
                        <p>PostgreSQL</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/MariaDB.png" alt="maria_db icon" />
                        <p>MariaDB</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/Mysql.png" alt="mysql icon" />
                        <p>MYSQL</p>
                      </a>
                    </div>
                    <div class="single-tech">
                      <a href="#">
                        <img src="images/MongoDB.png" alt="mongodb icon" />
                        <p>MongoDB</p>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="mb-5">
      <h2 class="before-content">Faqs</h2>
      <h4 class="sub-heading mb-4">Do you have a Question for us?</h4>
      <div class="container">
        <div class="accordion" id="accordionExample">
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseOne"
                aria-expanded="true"
                aria-controls="collapseOne"
              >
                What services does your software agency provide?
              </button>
            </h2>
            <div
              id="collapseOne"
              class="accordion-collapse collapse show"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We provide highly skilled developers to companies, offshore
                firms, and businesses on a full-time, part-time, or
                project-based model. Our services include web development,
                mobile app development, API integrations, cloud solutions, and
                more.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseTwo"
                aria-expanded="false"
                aria-controls="collapseTwo"
              >
                How does your developer hiring model work?
              </button>
            </h2>
            <div
              id="collapseTwo"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We offer multiple engagement models: <br />
                <br />
                <p>
                  <strong>Dedicated Developer Model</strong>: Hire a developer
                  who works exclusively for your project.
                </p>
                <p>
                  <strong>Project-Based Model</strong>: We handle your entire
                  project from planning to execution.
                </p>
                <p>
                  <strong>Offshore Managed Team Solutions </strong>: Get a fully
                  managed offshore development team with expert developers,
                  seamless communication, and end-to-end project handling—ready
                  to start in just 24 hours!
                </p>
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseThree"
                aria-expanded="false"
                aria-controls="collapseThree"
              >
                What technologies and expertise do you offer?
              </button>
            </h2>
            <div
              id="collapseThree"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We specialize in: <br />
                <br />
                <p>
                  <strong>Frontend</strong>: React.js, Vue.js, Angular, Next.js
                </p>
                <p>
                  <strong>Backend</strong>: Laravel, Node.js, PHP, Python, Java
                </p>
                <p><strong>Mobile Apps</strong>: Flutter, React Native</p>
                <p>
                  <strong>Database</strong>: MySQL, PostgreSQL, Firebase,
                  MongoDB
                </p>
                <p>
                  <strong>Cloud & DevOps</strong>: AWS, Firebase, Docker,
                  Kubernetes
                </p>
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseFour"
                aria-expanded="false"
                aria-controls="collapseFour"
              >
                How do you ensure the quality of your developers?
              </button>
            </h2>
            <div
              id="collapseFour"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Our developers undergo a rigorous screening process, including
                technical assessments, project experience reviews, and soft
                skills evaluations to ensure they meet industry standards.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseFive"
                aria-expanded="false"
                aria-controls="collapseFive"
              >
                Can I interview the developers before hiring?
              </button>
            </h2>
            <div
              id="collapseFive"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Yes, we allow clients to conduct interviews to assess the skills
                and fit of our developers before finalizing the hiring process.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseSix"
                aria-expanded="false"
                aria-controls="collapseSix"
              >
                How do you manage offshore developers?
              </button>
            </h2>
            <div
              id="collapseSix"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We ensure seamless communication through tools like Slack, Zoom,
                Jira, and Trello. Our developers work in your preferred time
                zones and follow agile methodologies for efficient
                collaboration.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseSeven"
                aria-expanded="false"
                aria-controls="collapseSeven"
              >
                Do you offer trial periods for developers?
              </button>
            </h2>
            <div
              id="collapseSeven"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Yes, we provide a risk-free trial period so you can evaluate the
                developer's performance before making a long-term commitment.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseEight"
                aria-expanded="false"
                aria-controls="collapseEight"
              >
                What is your pricing model?
              </button>
            </h2>
            <div
              id="collapseEight"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Our pricing depends on the engagement model: <br />
                <br />
                <p><strong>Hourly Rate</strong>: Best for short-term tasks.</p>
                <p>
                  <strong>Monthly Retainer</strong>: Ideal for long-term
                  development work.
                </p>
                <p>
                  <strong>Fixed Price</strong>: Suitable for well-defined
                  projects with clear deliverables.
                </p>
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseTen"
                aria-expanded="false"
                aria-controls="collapseTen"
              >
                What payment methods do you accept?
              </button>
            </h2>
            <div
              id="collapseTen"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We accept payments via bank transfer, PayPal, Wise, and Payoneer
                (on request).
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseNine"
                aria-expanded="false"
                aria-controls="collapseNine"
              >
                Are there any hidden costs?
              </button>
            </h2>
            <div
              id="collapseNine"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                No, we provide transparent pricing with no hidden charges. The
                cost includes developer fees, management, and support.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseEleven"
                aria-expanded="false"
                aria-controls="collapseEleven"
              >
                Do you sign NDAs and contracts?
              </button>
            </h2>
            <div
              id="collapseEleven"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Yes, we sign NDAs and legal agreements to ensure confidentiality
                and security of your project.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseTwelve"
                aria-expanded="false"
                aria-controls="collapseTwelve"
              >
                How do you handle project management?
              </button>
            </h2>
            <div
              id="collapseTwelve"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We use agile methodologies and project management tools like
                Jira, Trello, and Clickup to ensure smooth execution and
                real-time collaboration.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseThirteen"
                aria-expanded="false"
                aria-controls="collapseThirteen"
              >
                What if I need to scale my team?
              </button>
            </h2>
            <div
              id="collapseThirteen"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                We provide flexible team scaling, allowing you to add or reduce
                developers based on your project needs.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseFourteen"
                aria-expanded="false"
                aria-controls="collapseFourteen"
              >
                Do you offer post-development support?
              </button>
            </h2>
            <div
              id="collapseFourteen"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Yes, we offer maintenance, bug fixes, and ongoing support to
                ensure your application runs smoothly.
              </div>
            </div>
          </div>
          <div class="accordion-item">
            <h2 class="accordion-header">
              <button
                class="accordion-button collapsed"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#collapseFivteen"
                aria-expanded="false"
                aria-controls="collapseFivteen"
              >
                Can you take over an existing project?
              </button>
            </h2>
            <div
              id="collapseFivteen"
              class="accordion-collapse collapse"
              data-bs-parent="#accordionExample"
            >
              <div class="accordion-body">
                Absolutely! Our team can assess your current project, fix
                issues, and continue development as per your requirements.
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <footer class="footer-bg text-light mt-3">
      <div class="fixed-shape">
        <img src="images/footer-shape.png" alt="Shape" />
      </div>
      <div class="container">
        <div class="f-items default-padding">
          <div class="row">
            <div class="col-lg-4 col-md-6 item">
              <div class="f-item about">
                <img src="images/logo.webp" alt="Logo" />
                <p>
                  Started as a one-man endeavor, we have now expanded into a
                  multinational software development company with a globalized
                  clientele. We use our technological and creative genius to
                  design and develop simple yet splendid digital experiences,
                  across all platforms.
                </p>
              </div>
            </div>

            <div class="col-lg-2 col-md-6 item">
              <div class="f-item link">
                <h4 class="widget-title">Company</h4>
                <div class="menu-company-container">
                  <ul class="menu">
                    <li class="menu-item">
                      <a href="https://quadacts.com/about-us/"> About Us</a>
                    </li>
                    <li class="menu-item">
                      <a href="https://quadacts.com/contact-us/">Contact Us</a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>

            <div class="col-lg-2 col-md-6 item">
              <div class="f-item link">
                <h4 class="widget-title">
                  HIRE DEDICATED REMOTE DEVELOPERS IN PAKISTAN
                </h4>
                <div class="menu-solutions-container">
                  <ul class="menu">
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-react-js-developers/"
                        >React Js</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-react-developers/"
                        >React Native</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-laravel-developers/"
                        >Laravel</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-typescript-developers/"
                        >Typescript</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-node-js-developers/"
                        >Node Js</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-wordpress-developers/"
                        >WordPress</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-php-developers/"
                        >PHP</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-html5-developers/"
                        >HTML5</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-devops-developers/"
                        >DevOps</a
                      >
                    </li>
                    <li class="menu-item">
                      <a href="https://quadacts.com/hire-dedicated-qa-testers/"
                        >Manual QA</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-mern-stack-developers/"
                        >MERN Stack</a
                      >
                    </li>
                    <li class="menu-item">
                      <a
                        href="https://quadacts.com/hire-dedicated-shopify-developers/"
                        >Shopify</a
                      >
                    </li>
                  </ul>
                </div>
              </div>
            </div>
            <div class="col-lg-4 col-md-6 item">
              <div class="f-item">
                <h4 class="widget-title">Contact Info</h4>
                <div class="address">
                  <ul class="footer-contact">
                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 137"
                          fill="none"
                        >
                          <path
                            d="M0 56.2651C0.211872 26.144 22.7174 2.00233 49.9783 0.142559C81.2647 -1.98793 106.289 20.0115 109.408 49.3558C111.174 65.9171 105.242 79.6652 96.5313 92.7307C85.8082 108.833 72.0012 122.169 57.5939 134.905C55.8872 136.412 54.6866 136.988 52.6855 135.211C34.9942 119.615 18.2092 103.242 7.03886 82.0665C2.58954 73.6504 0.141248 64.6576 0 56.2651ZM13.654 54.6761C13.9836 57.2657 14.0424 59.8434 14.6427 62.2917C20.4104 85.5506 36.7363 101.394 53.2858 117.061C54.5571 118.261 55.5576 118.097 56.7817 116.931C71.6716 102.912 86.0083 88.4226 93.5651 68.8833C99.4857 53.5932 94.9894 35.0191 82.1946 23.8252C68.8114 12.1133 53.227 10.5361 37.2542 17.6456C21.5168 24.6491 14.9958 38.1501 13.654 54.6761Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M55.2164 41.3047C62.4083 41.3988 68.7526 47.7197 68.7762 54.8056C68.7997 62.1388 62.1258 68.6244 54.722 68.4949C47.3065 68.3655 40.9974 61.7386 41.1975 54.2877C41.3976 47.2253 47.895 41.2105 55.2164 41.3047Z"
                            fill="#F8F9F9"
                          />
                        </svg>
                      </div>
                      <div class="info">
                        𝗨𝗦𝗔 𝗢𝗙𝗙𝗜𝗖𝗘: 919 North market street, suite 950,
                        wilmington, DE 19801
                      </div>
                    </li>
                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 110"
                          fill="none"
                        >
                          <path
                            d="M0.00428509 27.2167C0.321556 27.4911 0.351568 27.1824 0.379436 27.0066C1.02898 22.7921 3.27775 19.495 6.2854 16.6182C8.78712 14.2258 11.0638 11.6018 13.6662 9.31449C18.1938 5.33789 24.044 5.21998 28.5823 9.62962C33.3949 14.3051 38.2547 18.942 42.7201 23.954C46.1179 27.7698 46.0493 33.3649 42.7244 37.2686C40.3127 40.1005 37.6202 42.6579 34.9191 45.2025C34.3553 45.732 34.3724 46.0793 34.6854 46.7096C37.5816 52.5727 41.7576 57.4732 46.2508 62.1337C50.1524 66.181 54.4441 69.7696 59.1839 72.7987C60.4701 73.6197 61.8807 74.2414 63.2012 75.011C63.7136 75.309 63.8979 74.9638 64.1616 74.7044C66.2903 72.5907 68.4126 70.4727 70.5413 68.359C72.58 66.3332 74.8523 64.7426 77.8235 64.4467C80.6275 64.1659 83.1721 64.8733 85.2666 66.7277C89.599 70.5628 93.4963 74.8481 97.6144 78.9083C99.2115 80.4839 100.873 82.0188 101.874 84.0875C103.754 87.9655 103.36 91.7299 100.776 95.0462C97.7923 98.8749 94.1201 102.101 90.6816 105.52C88.6794 107.512 86.094 108.61 83.3415 109.257C82.8935 109.362 82.8956 109.371 82.7177 109.724C82.3597 109.724 82.0038 109.724 81.6458 109.724C81.4314 109.388 81.2171 109.465 81.0027 109.724C78.859 109.724 76.7174 109.724 74.5737 109.724C74.3764 109.353 74.3765 109.351 73.937 109.281C71.1566 108.83 68.434 108.144 65.7715 107.22C57.3038 104.277 49.5114 100.013 42.2099 94.8747C28.5265 85.2451 17.5442 73.1052 9.21158 58.6072C4.99059 51.2629 1.6228 43.5583 0.381582 35.0777C0.353714 34.8891 0.413738 34.529 0 34.724C0 34.366 0 34.0102 0 33.6522C0.0557368 33.6457 0.1522 33.6522 0.156488 33.6329C0.192931 33.5064 0.10933 33.4571 0 33.4378C0 32.8654 0 32.2952 0 31.7228C0.323702 31.2233 0.353714 30.7238 0 30.2222C0 29.5791 0 28.936 0 28.2929C0.0664554 28.2436 0.135055 28.1921 0.20151 28.1428C0.135055 28.1214 0.0664554 28.0999 0 28.0785C0.00428744 27.7869 0.00428509 27.5018 0.00428509 27.2167ZM6.40544 30.8889C6.34328 34.1452 7.01426 37.0928 7.89319 40.0019C10.6564 49.1363 15.229 57.3424 20.8263 65.0084C30.3958 78.1151 42.1649 88.7244 56.5643 96.3196C63.1734 99.8053 69.9625 102.779 77.5406 103.417C80.9062 103.7 84.0961 103.165 86.6021 100.785C89.6805 97.8631 92.6259 94.7954 95.5093 91.6806C97.2542 89.7941 97.1707 87.6354 95.4814 85.7146C94.4696 84.5634 93.3591 83.498 92.2744 82.4133C88.8444 78.9747 85.4252 75.5255 81.9631 72.117C79.9823 70.1662 77.7635 70.1726 75.7056 72.0377C75.3347 72.3721 74.981 72.7258 74.6273 73.0795C72.2263 75.4762 69.8382 77.8857 67.4265 80.2738C65.6965 81.9867 63.7028 82.3168 61.5613 81.2385C60.0993 80.5032 58.6351 79.7636 57.2288 78.9319C50.4139 74.8931 44.5551 69.7096 39.2601 63.8551C34.9856 59.1282 31.129 54.1161 28.5029 48.2552C27.1867 45.3183 27.5147 43.5883 29.8728 41.3674C32.5245 38.8678 35.142 36.3361 37.618 33.6629C39.4616 31.6714 39.4638 29.7206 37.7252 27.639C37.4508 27.311 37.1593 26.9959 36.857 26.6936C32.8483 22.672 28.8395 18.6504 24.8243 14.6352C22.1318 11.9427 19.868 11.9427 17.1755 14.6266C14.8731 16.9204 12.6265 19.2721 10.2684 21.508C7.50088 24.1276 6.19965 27.2875 6.40544 30.8889Z"
                            fill="#FBFCFB"
                          />
                          <path
                            d="M62.5754 0C62.3803 0.505918 62.8262 0.373007 63.0234 0.413738C72.9296 2.39239 81.721 6.69056 89.4105 13.2075C96.219 18.9762 101.516 25.9219 105.199 34.0702C107.036 38.1347 108.387 42.3557 109.266 46.7289C109.356 47.1747 109.362 47.1726 109.72 47.3591C109.72 47.5735 109.72 47.7879 109.72 48.0022C109.354 48.1459 109.516 48.2873 109.72 48.431C109.72 49.0033 109.72 49.5736 109.72 50.146C109.283 49.9166 109.315 50.3689 109.195 50.5383C108.481 51.5415 107.546 52.1267 106.299 51.9853C104.858 51.8202 103.87 51.0485 103.499 49.595C102.937 47.3891 102.53 45.1511 101.87 42.9645C99.6404 35.5858 95.9403 29.0474 90.8297 23.298C84.7436 16.4531 77.3671 11.5011 68.745 8.42054C66.0632 7.4623 63.2957 6.79346 60.5003 6.2661C59.0597 5.99385 58.0329 5.26069 57.7113 3.83083C57.3812 2.36238 57.9257 1.17476 59.2483 0.383726C59.3748 0.308696 59.5978 0.212228 59.3663 0.00428713C60.4338 -3.10655e-07 61.5057 0 62.5754 0Z"
                            fill="#FBFCFB"
                          />
                          <path
                            d="M89.7706 49.8179C89.7749 51.3528 88.6837 52.6605 87.2003 52.9135C85.7104 53.1686 84.2526 52.3261 83.7532 50.8533C83.1722 49.1362 82.9685 47.3141 82.3169 45.6119C78.2438 34.9919 70.6636 28.4879 59.5248 26.1469C57.4797 25.716 56.3864 24.164 56.8109 22.2711C57.1903 20.5754 58.9053 19.6172 60.826 20.0245C61.5228 20.1724 62.2345 20.2646 62.9162 20.4596C77.2277 24.5649 86.152 33.8236 89.5669 48.343C89.6484 48.6882 89.7106 49.0376 89.7684 49.3892C89.7899 49.5285 89.7706 49.6743 89.7706 49.8179Z"
                            fill="#FBFCFB"
                          />
                        </svg>
                      </div>
                      <div class="info">+1 (848) 236-6981</div>
                    </li>
                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 137"
                          fill="none"
                        >
                          <path
                            d="M0 56.2651C0.211872 26.144 22.7174 2.00233 49.9783 0.142559C81.2647 -1.98793 106.289 20.0115 109.408 49.3558C111.174 65.9171 105.242 79.6652 96.5313 92.7307C85.8082 108.833 72.0012 122.169 57.5939 134.905C55.8872 136.412 54.6866 136.988 52.6855 135.211C34.9942 119.615 18.2092 103.242 7.03886 82.0665C2.58954 73.6504 0.141248 64.6576 0 56.2651ZM13.654 54.6761C13.9836 57.2657 14.0424 59.8434 14.6427 62.2917C20.4104 85.5506 36.7363 101.394 53.2858 117.061C54.5571 118.261 55.5576 118.097 56.7817 116.931C71.6716 102.912 86.0083 88.4226 93.5651 68.8833C99.4857 53.5932 94.9894 35.0191 82.1946 23.8252C68.8114 12.1133 53.227 10.5361 37.2542 17.6456C21.5168 24.6491 14.9958 38.1501 13.654 54.6761Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M55.2164 41.3047C62.4083 41.3988 68.7526 47.7197 68.7762 54.8056C68.7997 62.1388 62.1258 68.6244 54.722 68.4949C47.3065 68.3655 40.9974 61.7386 41.1975 54.2877C41.3976 47.2253 47.895 41.2105 55.2164 41.3047Z"
                            fill="#F8F9F9"
                          />
                        </svg>
                      </div>
                      <div class="info">
                        𝗣𝗔𝗞𝗜𝗦𝗧𝗔𝗡 𝗢𝗙𝗙𝗜𝗖𝗘: 2nd Floor, Paris Plaza, AA-Block City
                        Housing Scheme , Gujranwala ,Pakistan
                      </div>
                    </li>
                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 110"
                          fill="none"
                        >
                          <path
                            d="M0.00428509 27.2167C0.321556 27.4911 0.351568 27.1824 0.379436 27.0066C1.02898 22.7921 3.27775 19.495 6.2854 16.6182C8.78712 14.2258 11.0638 11.6018 13.6662 9.31449C18.1938 5.33789 24.044 5.21998 28.5823 9.62962C33.3949 14.3051 38.2547 18.942 42.7201 23.954C46.1179 27.7698 46.0493 33.3649 42.7244 37.2686C40.3127 40.1005 37.6202 42.6579 34.9191 45.2025C34.3553 45.732 34.3724 46.0793 34.6854 46.7096C37.5816 52.5727 41.7576 57.4732 46.2508 62.1337C50.1524 66.181 54.4441 69.7696 59.1839 72.7987C60.4701 73.6197 61.8807 74.2414 63.2012 75.011C63.7136 75.309 63.8979 74.9638 64.1616 74.7044C66.2903 72.5907 68.4126 70.4727 70.5413 68.359C72.58 66.3332 74.8523 64.7426 77.8235 64.4467C80.6275 64.1659 83.1721 64.8733 85.2666 66.7277C89.599 70.5628 93.4963 74.8481 97.6144 78.9083C99.2115 80.4839 100.873 82.0188 101.874 84.0875C103.754 87.9655 103.36 91.7299 100.776 95.0462C97.7923 98.8749 94.1201 102.101 90.6816 105.52C88.6794 107.512 86.094 108.61 83.3415 109.257C82.8935 109.362 82.8956 109.371 82.7177 109.724C82.3597 109.724 82.0038 109.724 81.6458 109.724C81.4314 109.388 81.2171 109.465 81.0027 109.724C78.859 109.724 76.7174 109.724 74.5737 109.724C74.3764 109.353 74.3765 109.351 73.937 109.281C71.1566 108.83 68.434 108.144 65.7715 107.22C57.3038 104.277 49.5114 100.013 42.2099 94.8747C28.5265 85.2451 17.5442 73.1052 9.21158 58.6072C4.99059 51.2629 1.6228 43.5583 0.381582 35.0777C0.353714 34.8891 0.413738 34.529 0 34.724C0 34.366 0 34.0102 0 33.6522C0.0557368 33.6457 0.1522 33.6522 0.156488 33.6329C0.192931 33.5064 0.10933 33.4571 0 33.4378C0 32.8654 0 32.2952 0 31.7228C0.323702 31.2233 0.353714 30.7238 0 30.2222C0 29.5791 0 28.936 0 28.2929C0.0664554 28.2436 0.135055 28.1921 0.20151 28.1428C0.135055 28.1214 0.0664554 28.0999 0 28.0785C0.00428744 27.7869 0.00428509 27.5018 0.00428509 27.2167ZM6.40544 30.8889C6.34328 34.1452 7.01426 37.0928 7.89319 40.0019C10.6564 49.1363 15.229 57.3424 20.8263 65.0084C30.3958 78.1151 42.1649 88.7244 56.5643 96.3196C63.1734 99.8053 69.9625 102.779 77.5406 103.417C80.9062 103.7 84.0961 103.165 86.6021 100.785C89.6805 97.8631 92.6259 94.7954 95.5093 91.6806C97.2542 89.7941 97.1707 87.6354 95.4814 85.7146C94.4696 84.5634 93.3591 83.498 92.2744 82.4133C88.8444 78.9747 85.4252 75.5255 81.9631 72.117C79.9823 70.1662 77.7635 70.1726 75.7056 72.0377C75.3347 72.3721 74.981 72.7258 74.6273 73.0795C72.2263 75.4762 69.8382 77.8857 67.4265 80.2738C65.6965 81.9867 63.7028 82.3168 61.5613 81.2385C60.0993 80.5032 58.6351 79.7636 57.2288 78.9319C50.4139 74.8931 44.5551 69.7096 39.2601 63.8551C34.9856 59.1282 31.129 54.1161 28.5029 48.2552C27.1867 45.3183 27.5147 43.5883 29.8728 41.3674C32.5245 38.8678 35.142 36.3361 37.618 33.6629C39.4616 31.6714 39.4638 29.7206 37.7252 27.639C37.4508 27.311 37.1593 26.9959 36.857 26.6936C32.8483 22.672 28.8395 18.6504 24.8243 14.6352C22.1318 11.9427 19.868 11.9427 17.1755 14.6266C14.8731 16.9204 12.6265 19.2721 10.2684 21.508C7.50088 24.1276 6.19965 27.2875 6.40544 30.8889Z"
                            fill="#FBFCFB"
                          />
                          <path
                            d="M62.5754 0C62.3803 0.505918 62.8262 0.373007 63.0234 0.413738C72.9296 2.39239 81.721 6.69056 89.4105 13.2075C96.219 18.9762 101.516 25.9219 105.199 34.0702C107.036 38.1347 108.387 42.3557 109.266 46.7289C109.356 47.1747 109.362 47.1726 109.72 47.3591C109.72 47.5735 109.72 47.7879 109.72 48.0022C109.354 48.1459 109.516 48.2873 109.72 48.431C109.72 49.0033 109.72 49.5736 109.72 50.146C109.283 49.9166 109.315 50.3689 109.195 50.5383C108.481 51.5415 107.546 52.1267 106.299 51.9853C104.858 51.8202 103.87 51.0485 103.499 49.595C102.937 47.3891 102.53 45.1511 101.87 42.9645C99.6404 35.5858 95.9403 29.0474 90.8297 23.298C84.7436 16.4531 77.3671 11.5011 68.745 8.42054C66.0632 7.4623 63.2957 6.79346 60.5003 6.2661C59.0597 5.99385 58.0329 5.26069 57.7113 3.83083C57.3812 2.36238 57.9257 1.17476 59.2483 0.383726C59.3748 0.308696 59.5978 0.212228 59.3663 0.00428713C60.4338 -3.10655e-07 61.5057 0 62.5754 0Z"
                            fill="#FBFCFB"
                          />
                          <path
                            d="M89.7706 49.8179C89.7749 51.3528 88.6837 52.6605 87.2003 52.9135C85.7104 53.1686 84.2526 52.3261 83.7532 50.8533C83.1722 49.1362 82.9685 47.3141 82.3169 45.6119C78.2438 34.9919 70.6636 28.4879 59.5248 26.1469C57.4797 25.716 56.3864 24.164 56.8109 22.2711C57.1903 20.5754 58.9053 19.6172 60.826 20.0245C61.5228 20.1724 62.2345 20.2646 62.9162 20.4596C77.2277 24.5649 86.152 33.8236 89.5669 48.343C89.6484 48.6882 89.7106 49.0376 89.7684 49.3892C89.7899 49.5285 89.7706 49.6743 89.7706 49.8179Z"
                            fill="#FBFCFB"
                          />
                        </svg>
                      </div>
                      <div class="info">+92 316 765 2340</div>
                    </li>
                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 137"
                          fill="none"
                        >
                          <path
                            d="M0 56.2651C0.211872 26.144 22.7174 2.00233 49.9783 0.142559C81.2647 -1.98793 106.289 20.0115 109.408 49.3558C111.174 65.9171 105.242 79.6652 96.5313 92.7307C85.8082 108.833 72.0012 122.169 57.5939 134.905C55.8872 136.412 54.6866 136.988 52.6855 135.211C34.9942 119.615 18.2092 103.242 7.03886 82.0665C2.58954 73.6504 0.141248 64.6576 0 56.2651ZM13.654 54.6761C13.9836 57.2657 14.0424 59.8434 14.6427 62.2917C20.4104 85.5506 36.7363 101.394 53.2858 117.061C54.5571 118.261 55.5576 118.097 56.7817 116.931C71.6716 102.912 86.0083 88.4226 93.5651 68.8833C99.4857 53.5932 94.9894 35.0191 82.1946 23.8252C68.8114 12.1133 53.227 10.5361 37.2542 17.6456C21.5168 24.6491 14.9958 38.1501 13.654 54.6761Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M55.2164 41.3047C62.4083 41.3988 68.7526 47.7197 68.7762 54.8056C68.7997 62.1388 62.1258 68.6244 54.722 68.4949C47.3065 68.3655 40.9974 61.7386 41.1975 54.2877C41.3976 47.2253 47.895 41.2105 55.2164 41.3047Z"
                            fill="#F8F9F9"
                          />
                        </svg>
                      </div>
                      <div class="info">
                        𝗨𝗔𝗘 𝗢𝗙𝗙𝗜𝗖𝗘: Arab Bank Building, 1st Floor, Dubai, United
                        Arab Emirates
                      </div>
                    </li>
                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 110"
                          fill="none"
                        >
                          <path
                            d="M0.00428509 27.2167C0.321556 27.4911 0.351568 27.1824 0.379436 27.0066C1.02898 22.7921 3.27775 19.495 6.2854 16.6182C8.78712 14.2258 11.0638 11.6018 13.6662 9.31449C18.1938 5.33789 24.044 5.21998 28.5823 9.62962C33.3949 14.3051 38.2547 18.942 42.7201 23.954C46.1179 27.7698 46.0493 33.3649 42.7244 37.2686C40.3127 40.1005 37.6202 42.6579 34.9191 45.2025C34.3553 45.732 34.3724 46.0793 34.6854 46.7096C37.5816 52.5727 41.7576 57.4732 46.2508 62.1337C50.1524 66.181 54.4441 69.7696 59.1839 72.7987C60.4701 73.6197 61.8807 74.2414 63.2012 75.011C63.7136 75.309 63.8979 74.9638 64.1616 74.7044C66.2903 72.5907 68.4126 70.4727 70.5413 68.359C72.58 66.3332 74.8523 64.7426 77.8235 64.4467C80.6275 64.1659 83.1721 64.8733 85.2666 66.7277C89.599 70.5628 93.4963 74.8481 97.6144 78.9083C99.2115 80.4839 100.873 82.0188 101.874 84.0875C103.754 87.9655 103.36 91.7299 100.776 95.0462C97.7923 98.8749 94.1201 102.101 90.6816 105.52C88.6794 107.512 86.094 108.61 83.3415 109.257C82.8935 109.362 82.8956 109.371 82.7177 109.724C82.3597 109.724 82.0038 109.724 81.6458 109.724C81.4314 109.388 81.2171 109.465 81.0027 109.724C78.859 109.724 76.7174 109.724 74.5737 109.724C74.3764 109.353 74.3765 109.351 73.937 109.281C71.1566 108.83 68.434 108.144 65.7715 107.22C57.3038 104.277 49.5114 100.013 42.2099 94.8747C28.5265 85.2451 17.5442 73.1052 9.21158 58.6072C4.99059 51.2629 1.6228 43.5583 0.381582 35.0777C0.353714 34.8891 0.413738 34.529 0 34.724C0 34.366 0 34.0102 0 33.6522C0.0557368 33.6457 0.1522 33.6522 0.156488 33.6329C0.192931 33.5064 0.10933 33.4571 0 33.4378C0 32.8654 0 32.2952 0 31.7228C0.323702 31.2233 0.353714 30.7238 0 30.2222C0 29.5791 0 28.936 0 28.2929C0.0664554 28.2436 0.135055 28.1921 0.20151 28.1428C0.135055 28.1214 0.0664554 28.0999 0 28.0785C0.00428744 27.7869 0.00428509 27.5018 0.00428509 27.2167ZM6.40544 30.8889C6.34328 34.1452 7.01426 37.0928 7.89319 40.0019C10.6564 49.1363 15.229 57.3424 20.8263 65.0084C30.3958 78.1151 42.1649 88.7244 56.5643 96.3196C63.1734 99.8053 69.9625 102.779 77.5406 103.417C80.9062 103.7 84.0961 103.165 86.6021 100.785C89.6805 97.8631 92.6259 94.7954 95.5093 91.6806C97.2542 89.7941 97.1707 87.6354 95.4814 85.7146C94.4696 84.5634 93.3591 83.498 92.2744 82.4133C88.8444 78.9747 85.4252 75.5255 81.9631 72.117C79.9823 70.1662 77.7635 70.1726 75.7056 72.0377C75.3347 72.3721 74.981 72.7258 74.6273 73.0795C72.2263 75.4762 69.8382 77.8857 67.4265 80.2738C65.6965 81.9867 63.7028 82.3168 61.5613 81.2385C60.0993 80.5032 58.6351 79.7636 57.2288 78.9319C50.4139 74.8931 44.5551 69.7096 39.2601 63.8551C34.9856 59.1282 31.129 54.1161 28.5029 48.2552C27.1867 45.3183 27.5147 43.5883 29.8728 41.3674C32.5245 38.8678 35.142 36.3361 37.618 33.6629C39.4616 31.6714 39.4638 29.7206 37.7252 27.639C37.4508 27.311 37.1593 26.9959 36.857 26.6936C32.8483 22.672 28.8395 18.6504 24.8243 14.6352C22.1318 11.9427 19.868 11.9427 17.1755 14.6266C14.8731 16.9204 12.6265 19.2721 10.2684 21.508C7.50088 24.1276 6.19965 27.2875 6.40544 30.8889Z"
                            fill="#FBFCFB"
                          />
                          <path
                            d="M62.5754 0C62.3803 0.505918 62.8262 0.373007 63.0234 0.413738C72.9296 2.39239 81.721 6.69056 89.4105 13.2075C96.219 18.9762 101.516 25.9219 105.199 34.0702C107.036 38.1347 108.387 42.3557 109.266 46.7289C109.356 47.1747 109.362 47.1726 109.72 47.3591C109.72 47.5735 109.72 47.7879 109.72 48.0022C109.354 48.1459 109.516 48.2873 109.72 48.431C109.72 49.0033 109.72 49.5736 109.72 50.146C109.283 49.9166 109.315 50.3689 109.195 50.5383C108.481 51.5415 107.546 52.1267 106.299 51.9853C104.858 51.8202 103.87 51.0485 103.499 49.595C102.937 47.3891 102.53 45.1511 101.87 42.9645C99.6404 35.5858 95.9403 29.0474 90.8297 23.298C84.7436 16.4531 77.3671 11.5011 68.745 8.42054C66.0632 7.4623 63.2957 6.79346 60.5003 6.2661C59.0597 5.99385 58.0329 5.26069 57.7113 3.83083C57.3812 2.36238 57.9257 1.17476 59.2483 0.383726C59.3748 0.308696 59.5978 0.212228 59.3663 0.00428713C60.4338 -3.10655e-07 61.5057 0 62.5754 0Z"
                            fill="#FBFCFB"
                          />
                          <path
                            d="M89.7706 49.8179C89.7749 51.3528 88.6837 52.6605 87.2003 52.9135C85.7104 53.1686 84.2526 52.3261 83.7532 50.8533C83.1722 49.1362 82.9685 47.3141 82.3169 45.6119C78.2438 34.9919 70.6636 28.4879 59.5248 26.1469C57.4797 25.716 56.3864 24.164 56.8109 22.2711C57.1903 20.5754 58.9053 19.6172 60.826 20.0245C61.5228 20.1724 62.2345 20.2646 62.9162 20.4596C77.2277 24.5649 86.152 33.8236 89.5669 48.343C89.6484 48.6882 89.7106 49.0376 89.7684 49.3892C89.7899 49.5285 89.7706 49.6743 89.7706 49.8179Z"
                            fill="#FBFCFB"
                          />
                        </svg>
                      </div>
                      <div class="info">+971 558 740 397</div>
                    </li>

                    <li>
                      <div class="icon">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="24"
                          height="24"
                          viewBox="0 0 110 118"
                          fill="none"
                        >
                          <path
                            d="M101.151 111.592C95.0483 106.633 89.3725 101.987 83.6586 97.3794C74.7636 90.2084 65.8532 83.0527 56.9429 75.897C55.9054 75.0578 55.1807 73.9288 53.4795 75.3172C38.9315 87.2027 24.2844 98.9814 9.68309 110.806C9.43134 111.012 9.3398 111.409 9.17197 111.721C10.4612 112.82 11.9259 112.37 13.2686 112.377C24.5743 112.416 35.8801 112.4 47.1934 112.4C48.208 112.4 49.2227 112.438 50.2373 112.469C52.0605 112.53 53.2201 113.598 53.1743 115.284C53.1286 116.947 51.9766 117.977 50.1152 117.962C46.6823 117.939 43.257 117.977 39.8241 117.977C30.0441 117.977 20.2565 118.03 10.4765 117.954C4.40403 117.908 0.208239 114.185 0.14721 108.128C-0.0358793 89.0718 0.0404077 70.0153 0.124323 50.9588C0.139581 48.4031 1.29152 46.229 3.18343 44.2455C8.89733 38.2875 14.3213 32.0625 19.9742 26.0434C21.1567 24.777 21.3779 23.4954 21.3626 21.9315C21.3245 16.3397 21.3474 10.7479 21.3474 5.16364C21.3474 3.37853 21.0651 1.14331 23.2774 0.738993C25.9322 0.250756 28.8311 -0.351913 31.3715 1.33403C32.3098 1.95958 32.2946 3.11152 31.9589 4.12614C31.547 5.36199 30.5552 5.98755 29.3117 5.87312C27.1223 5.66714 26.8629 6.82671 26.8706 8.62708C26.9163 25.7764 26.924 42.9181 26.8553 60.0674C26.8477 61.9365 27.5419 63.1418 28.9379 64.248C33.2176 67.6351 37.505 71.0222 41.6321 74.5925C43.196 75.9428 44.1496 75.569 45.4693 74.4094C47.6588 72.487 50.0466 70.7858 52.236 68.8557C54.1127 67.2003 55.8139 67.2231 57.5913 68.8938C57.7744 69.0693 57.9957 69.2066 58.2016 69.3516C61.0624 71.3961 63.4807 75.5537 66.6161 75.2638C69.4616 75.0044 71.7884 71.0222 74.7178 69.3058C81.2403 65.4762 83.2238 60.2353 82.8652 52.66C82.1863 38.2112 82.5982 23.7166 82.7279 9.23737C82.7508 6.55969 81.9574 5.82734 79.3103 5.85786C66.8602 5.98755 54.4102 5.91889 41.9601 5.91889C40.9455 5.91889 39.9233 5.91888 38.9086 5.88837C37.1006 5.83497 35.9258 4.87376 35.9792 3.09627C36.0326 1.31878 37.2837 0.44911 39.0689 0.44911C41.0981 0.441481 43.135 0.426225 45.1642 0.426225C57.9957 0.426225 70.8271 0.494873 83.6586 0.380443C87.1068 0.349928 88.5944 1.6163 88.3808 5.0721C88.2435 7.35308 88.6478 9.68746 88.3045 11.9227C86.9313 20.8025 89.7692 27.798 96.635 33.6263C100.48 36.8914 103.554 41.0491 107.071 44.7109C109.07 46.7935 109.703 49.3033 109.711 52.0039C109.741 70.4272 109.795 88.8505 109.695 107.274C109.657 114.018 105.66 117.931 99.0228 117.962C86.321 118.023 73.6116 117.962 60.9098 118C58.8577 118.007 56.7293 117.824 56.6454 115.376C56.5462 112.66 58.8043 112.385 60.9785 112.393C72.7954 112.4 84.6122 112.408 96.4291 112.385C97.7793 112.377 99.2059 112.66 101.151 111.592ZM5.86111 106.671C16.8236 97.7685 27.2367 89.2777 37.6957 80.8557C39.4732 79.4215 38.1534 78.8646 37.1922 78.0788C30.7917 72.8836 24.3989 67.6809 17.9907 62.4934C14.1154 59.3503 10.2247 56.2302 5.86874 52.7134C5.86111 70.9154 5.86111 88.3623 5.86111 106.671ZM103.882 106.587C103.882 88.3852 103.882 70.8773 103.882 52.7668C92.6529 61.8678 81.8659 70.6179 70.7127 79.6579C81.7286 88.6064 92.5766 97.4099 103.882 106.587ZM88.5562 57.9772C92.7749 54.5595 96.4977 51.5385 100.236 48.5328C101.09 47.8462 101.762 47.297 100.694 46.1527C96.7647 41.9416 92.9046 37.6619 88.5562 32.9092C88.5562 41.5373 88.5562 49.4254 88.5562 57.9772ZM21.1185 57.8322C21.1185 49.3872 21.1185 41.522 21.1185 33.0847C16.9685 37.6162 13.2686 41.7509 9.4466 45.7712C8.21838 47.0605 8.50064 47.7394 9.75938 48.7235C13.4517 51.5843 17.0677 54.5671 21.1185 57.8322Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M55.2036 9.46629C61.0395 9.46629 66.8755 9.58072 72.7115 9.42052C76.0833 9.32898 77.693 10.5496 77.5328 14.0359C77.4031 16.9501 77.5328 19.8718 77.487 22.786C77.4489 25.2425 76.2359 26.4402 73.6727 26.4249C61.1158 26.3791 48.559 26.3868 35.9945 26.4249C33.4618 26.4325 32.1954 25.2882 32.1725 22.8013C32.142 19.6277 32.1801 16.4618 32.1573 13.2883C32.1344 10.5572 33.5228 9.43578 36.1776 9.45867C42.5247 9.50444 48.8641 9.46629 55.2036 9.46629ZM54.6238 20.917C59.4451 20.917 64.2665 20.7644 69.0802 20.978C71.7731 21.1001 71.796 19.7422 71.8647 17.7434C71.9333 15.6379 71.3841 14.9056 69.1565 14.9361C59.6435 15.0581 50.1305 15.0429 40.6251 14.9437C38.5272 14.9208 37.6881 15.4396 37.8178 17.6671C37.9322 19.5972 37.833 21.0924 40.5565 20.978C45.2329 20.7797 49.9321 20.917 54.6238 20.917Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M67.1578 97.4863C66.9747 104.062 61.3753 109.471 54.7993 109.41C47.9259 109.349 42.4561 103.559 42.6315 96.5327C42.7994 89.9491 48.4065 84.5327 54.99 84.609C61.9245 84.6853 67.3562 90.4297 67.1578 97.4863ZM61.6728 97.1735C61.7796 93.5117 58.7129 90.3916 54.9672 90.3534C51.2138 90.3153 48.0937 93.3439 48.1013 97.0209C48.109 100.553 51.0384 103.528 54.6239 103.65C58.4153 103.78 61.566 100.881 61.6728 97.1735Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M48.8414 41.7815C52.5261 41.7815 56.2107 41.7892 59.8954 41.7815C61.856 41.7739 63.6792 42.3232 63.7555 44.5355C63.8394 47.0224 61.8941 47.5641 59.8038 47.5641C52.5642 47.5641 45.3169 47.5793 38.0773 47.5564C36.2006 47.5488 34.3316 47.2208 34.2629 44.8254C34.1867 42.2087 36.1091 41.8044 38.1688 41.7968C41.7238 41.7663 45.2864 41.7815 48.8414 41.7815Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M60.9861 36.3575C57.187 36.3575 53.3955 36.327 49.5964 36.3727C47.5672 36.3956 45.9194 35.7548 45.9347 33.5349C45.9576 31.5361 47.5138 30.8724 49.3905 30.8801C56.981 30.903 64.5716 30.8877 72.1698 30.8953C74.0464 30.8953 75.5035 31.5895 75.4272 33.6951C75.351 35.7701 73.8557 36.3651 71.9943 36.3575C68.3249 36.3422 64.6555 36.3575 60.9861 36.3575Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M60.4979 58.4655C56.8284 58.4655 53.159 58.4349 49.4896 58.4807C47.4299 58.5036 45.8431 57.7865 45.9499 55.5589C46.0491 53.5602 47.6359 53.0186 49.5049 53.0262C56.9658 53.0643 64.4343 53.0491 71.8951 53.0338C73.7108 53.0262 75.3281 53.4229 75.412 55.5589C75.5111 57.8857 73.9015 58.4883 71.8799 58.4731C68.0884 58.4502 64.2893 58.4655 60.4979 58.4655Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M70.8576 47.5793C68.5843 47.5182 66.0058 47.6479 66.0973 44.5812C66.1889 41.5678 68.775 41.7891 70.9339 41.7815C73.0242 41.7738 75.351 41.812 75.4044 44.5735C75.4806 47.6784 72.9174 47.4801 70.8576 47.5793Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M38.9927 30.8572C41.3805 30.8724 43.6081 30.8343 43.5471 33.5348C43.4708 37.0593 40.5947 36.2659 38.4282 36.3575C36.4295 36.4414 34.2477 36.3346 34.2934 33.6874C34.3468 30.1401 37.2991 31.086 38.9927 30.8572Z"
                            fill="#F8F9F9"
                          />
                          <path
                            d="M38.9316 58.4884C36.788 58.4426 34.1713 58.8393 34.301 55.5666C34.4155 52.5761 36.9329 53.0415 38.9088 53.0338C41.0372 53.0262 43.6767 52.6143 43.5318 55.9175C43.4097 58.8851 40.9304 58.4426 38.9316 58.4884Z"
                            fill="#F8F9F9"
                          />
                        </svg>
                      </div>
                      <div class="info">
                        <a href="mailto:http://Info@quadacts.com"
                          >Info@quadacts.com</a
                        >
                        <br />
                        <a href="mailto:http://support@quadacts.com"
                          >support@quadacts.com</a
                        >
                      </div>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="footer-bottom">
        <div class="container">
          <div class="row">
            <div class="col-md-6">
              <p class="m-0">© Copyright 2021. Quadacts</p>
            </div>
            <div class="col-md-6 text-right link"></div>
          </div>
        </div>
      </div>
    </footer>
    <div
      class="modal fade"
      id="myModal"
      tabindex="-1"
      aria-labelledby="myModalLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
              aria-label="Close"
            ></button>
          </div>
          <div class="modal-body">
            <div class="popup-title">
              <h3 class="text-center">
                Looking for vetted developer / offshore development center?
              </h3>
              <p class="text-center">
                Connect with QuadActs experts to hire remote developer on
                contractual basis.
              </p>
            </div>
            <div class="container popup-wrapper">
              <div class="right-content">
                <div>
                  <div class="funnel">
                    <form
                      id="multiStepForm"
                      onsubmit="event.preventDefault();"
                      class="h-100"
                    >
                      <div class="d-flex justify-content-center">
                        <div class="progress-completion">
                          <span class="text-muted me-2">Completion</span>
                          <div class="lp-completion__progress-meter">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 34 34"
                            >
                              <circle
                                cx="17"
                                cy="17"
                                r="15.5"
                                class="progress-meter-bar"
                              ></circle>
                              <circle
                                id="js-countdown__progress"
                                cx="17"
                                cy="17"
                                r="15.5"
                                class="progress-meter-progress js-countdown__progress"
                                stroke-linecap="round"
                              ></circle>
                            </svg>
                            <span class="lp-completion-rate">0%</span>
                          </div>
                        </div>
                      </div>
                      <div class="step active">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              What Type of Developers Do You Need?
                            </h2>
                            <div
                              class="d-flex justify-content-center flex-column mt-3"
                            >
                              <div class="d-flex justify-content-center">
                                <div class="d-grid gap-2 px-4">
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="developerType"
                                      class="d-none"
                                      onclick="handleDeveloperTypeSelection(this)"
                                    />
                                    <span>Full Stack Web Developer</span>
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="developerType"
                                      class="d-none"
                                      onclick="handleDeveloperTypeSelection(this)"
                                    />
                                    <span
                                      >Frontend Developer (React, Vue, Angular
                                      etc)</span
                                    >
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="developerType"
                                      class="d-none"
                                      onclick="handleDeveloperTypeSelection(this)"
                                    />
                                    <span
                                      >Backend Developer (Node.js, Laravel,
                                      Python, etc.)</span
                                    >
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="developerType"
                                      class="d-none"
                                      onclick="handleDeveloperTypeSelection(this)"
                                    />
                                    <span
                                      >Mobile App Developer (Flutter, React
                                      Native)</span
                                    >
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="developerType"
                                      class="d-none"
                                      onclick="handleDeveloperTypeSelection(this)"
                                    />
                                    <span>AI/ML Developer</span>
                                  </label>
                                  <label class="btn-radio" id="otherOption">
                                    <input
                                      type="radio"
                                      name="developerType"
                                      class="d-none"
                                      onclick="handleDeveloperTypeSelection(this)"
                                    />
                                    <span>Other (Custom Needs)</span>
                                  </label>

                                  <div
                                    id="customNeedsInput"
                                    class="mt-2"
                                    style="display: none"
                                  >
                                    <input
                                      type="text"
                                      class="form-control customdata"
                                      placeholder="Please specify your custom needs"
                                      id="customNeedsText"
                                    />
                                    <small
                                      class="text-muted d-block mt-1 text-center"
                                      >Press Enter to proceed to the next
                                      step</small
                                    >
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-center align-items-center mb-2"
                          >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              What Hiring Model you want to choose ?
                            </h2>
                            <div class="d-flex justify-content-center mt-3">
                              <div class="d-grid gap-2 px-4">
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hiringType"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>Hourly</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hiringType"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>Part-Time (Weekly)</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hiringType"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>Full-Time (Monthly)</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hiringType"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>Project-Based (Fixed Price)</span>
                                </label>
                              </div>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>

                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              What's Your Expected Budget?
                            </h2>
                            <div class="d-flex justify-content-center gap-3">
                              <label class="hourly-radio">
                                <input
                                  type="radio"
                                  value="hourly"
                                  name="billingType"
                                  onchange="toggleBudgetOptions()"
                                  checked
                                />
                                <h3 class="text-center">Hourly</h3>
                              </label>
                              <label class="monthly-radio">
                                <input
                                  type="radio"
                                  value="monthly"
                                  name="billingType"
                                  onchange="toggleBudgetOptions()"
                                />
                                <h3 class="text-center">Monthly</h3>
                              </label>
                            </div>
                            <div id="hourlyBudgetOptions">
                              <div class="d-flex justify-content-center mt-3">
                                <div class="d-grid gap-2 px-4">
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleOptionSelect('hourly')"
                                      data-value="10-20"
                                    />
                                    <span>$10 - $20</span>
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleOptionSelect('hourly')"
                                      data-value="20-40"
                                    />
                                    <span>$20 - $40</span>
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleOptionSelect('hourly')"
                                      data-value="40-60"
                                    />
                                    <span>$40 - $60</span>
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleCustomSelect('hourly')"
                                      data-value="custom"
                                    />
                                    <span>Custom</span>
                                  </label>
                                </div>
                              </div>
                              <div
                                id="hourlyCustomInput"
                                class="text-center mt-2"
                                style="display: none"
                              >
                                <input
                                  type="number"
                                  id="hourlyCustomValue"
                                  class="form-control w-50 mx-auto customdata"
                                  placeholder="Enter custom hourly rate"
                                />
                                <small
                                  class="text-muted d-block mt-1 text-center"
                                  >Press Enter to proceed to the next
                                  step</small
                                >
                              </div>
                            </div>

                            <div
                              id="monthlyBudgetOptions"
                              style="display: none"
                            >
                              <div class="d-flex justify-content-center mt-3">
                                <div class="d-grid gap-2 px-4">
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleOptionSelect('monthly')"
                                      data-value="2000-4000"
                                    />
                                    <span>$2,000 - $4,000</span>
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleOptionSelect('monthly')"
                                      data-value="4000-6000"
                                    />
                                    <span>$4,000 - $6,000</span>
                                  </label>
                                  <label class="btn-radio">
                                    <input
                                      type="radio"
                                      name="budget"
                                      class="d-none"
                                      onclick="handleCustomSelect('monthly')"
                                      data-value="custom"
                                    />
                                    <span>Custom</span>
                                  </label>
                                </div>
                              </div>
                              <div
                                id="monthlyCustomInput"
                                class="text-center mt-2"
                                style="display: none"
                              >
                                <input
                                  type="number"
                                  id="monthlyCustomValue"
                                  class="form-control w-50 mx-auto customdata"
                                  placeholder="Enter custom monthly budget"
                                />
                                <small
                                  class="text-muted d-block mt-1 text-center"
                                  >Press Enter to proceed to the next
                                  step</small
                                >
                              </div>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              When Do You Want to Start?
                            </h2>
                            <div class="d-flex justify-content-center mt-3">
                              <div class="d-grid gap-2 px-4">
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="stating"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>ASAP</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="stating"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>In 1-2 Weeks</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="stating"
                                    class="d-none"
                                    onclick="nextStep()"
                                  />
                                  <span>In 1 Month</span>
                                </label>
                              </div>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              How many developers you want to Hire?
                            </h2>
                            <div class="d-flex justify-content-center mt-3">
                              <div class="d-grid gap-2 px-4">
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hireDevelopers"
                                    class="d-none"
                                    onclick="handleDeveloperSelection(this)"
                                  />
                                  <span>1 to 5</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hireDevelopers"
                                    class="d-none"
                                    onclick="handleDeveloperSelection(this)"
                                  />
                                  <span>6 to 10</span>
                                </label>
                                <label class="btn-radio">
                                  <input
                                    type="radio"
                                    name="hireDevelopers"
                                    class="d-none"
                                    onclick="handleDeveloperSelection(this)"
                                  />
                                  <span>11 +</span>
                                </label>
                                <label
                                  class="btn-radio"
                                  id="customDeveloperOption"
                                >
                                  <input
                                    type="radio"
                                    name="hireDevelopers"
                                    class="d-none"
                                    onclick="handleDeveloperSelection(this)"
                                  />
                                  <span>Custom</span>
                                </label>
                                <div
                                  id="customDeveloperInput"
                                  class="mt-2"
                                  style="display: none"
                                >
                                  <input
                                    type="number"
                                    class="form-control customdata"
                                    placeholder="Enter custom number of developers"
                                    id="customDeveloperCount"
                                    min="1"
                                  />
                                  <small
                                    class="text-muted d-block mt-1 text-center"
                                    >Press Enter to proceed to the next
                                    step</small
                                  >
                                </div>
                              </div>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">Enter Your Full Name</h2>
                            <div class="d-flex justify-content-center mt-3">
                              <input
                                type="text"
                                placeholder="Your Full Name"
                                class="form-control"
                                id="fullName"
                              />
                            </div>
                            <div
                              id="nameError"
                              class="text-danger text-center mt-2"
                              style="display: none"
                            >
                              Please enter your full name (first and last name)
                            </div>
                            <div class="d-flex justify-content-center mt-4">
                              <button
                                class="funnel-btn"
                                onclick="validateName()"
                              >
                                Next
                              </button>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              Enter Your Email Address
                            </h2>

                            <div class="d-flex justify-content-center mt-3">
                              <input
                                type="email"
                                placeholder="Your Email Address"
                                class="form-control"
                                id="emailAddress"
                              />
                            </div>
                            <div
                              id="emailError"
                              class="text-danger text-center mt-2"
                              style="display: none"
                            >
                              Please enter a valid email address
                            </div>
                            <div class="d-flex justify-content-center mt-4">
                              <button
                                class="funnel-btn"
                                onclick="validateEmail()"
                              >
                                Next
                              </button>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              What's your phone number?
                            </h2>
                            <div class="d-flex justify-content-center mt-3">
                              <input
                                type="number"
                                placeholder="Enter Your Phone Number"
                                id="country_code"
                                class="form-control"
                              />
                            </div>
                            <div class="d-flex justify-content-center mt-4">
                              <button class="funnel-btn" onclick="nextStep()">
                                Next
                              </button>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="step">
                        <div class="spacing">
                          <div>
                            <h2 class="text-center">
                              Enter Your Comapany Name
                            </h2>
                            <div class="d-flex justify-content-center mt-3">
                              <input
                                type="text"
                                placeholder="Company Name & Website"
                                class="form-control"
                              />
                            </div>
                            <div class="d-flex justify-content-center mt-4">
                              <button class="funnel-btn">
                                <a href="/hire-developers.php">
                                  See Available Developers
                                </a>
                              </button>
                            </div>
                          </div>
                          <div
                            class="d-flex justify-content-between align-items-center mb-2"
                          >
                            <a
                              href="javascript:void(0)"
                              class="text-decoration-none text-primary"
                              onclick="prevStep()"
                              >← Back</a
                            >
                            <div
                              class="d-flex justify-content-center align-items-center"
                            >
                              <img src="images/lock.png" height="20" alt="" />
                              <p class="d-block text-muted text-center mt-3">
                                Privacy & Security Guaranteed
                              </p>
                            </div>
                          </div>
                        </div>
                      </div>
                    </form>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <script>
      let currentStep = 0;
      let selectedOption = null;
      let selectedDeveloperCount = null;

      function handleDeveloperTypeSelection(element) {
        document.querySelectorAll(".btn-radio").forEach((btn) => {
          btn.classList.remove("active");
        });
        element.closest(".btn-radio").classList.add("active");
        const isOtherOption =
          element.closest(".btn-radio").id === "otherOption";
        const customInput = document.getElementById("customNeedsInput");
        const customText = document.getElementById("customNeedsText");

        if (isOtherOption) {
          customInput.style.display = "block";
          selectedOption = "Other (Custom Needs)";
        } else {
          customInput.style.display = "none";
          customText.value = "";
          selectedOption = element.closest(".btn-radio").textContent.trim();
          nextStep();
        }
      }

      function handleDeveloperSelection(element) {
        document.querySelectorAll(".btn-radio").forEach((btn) => {
          btn.classList.remove("active");
        });

        element.closest(".btn-radio").classList.add("active");

        const customInput = document.getElementById("customDeveloperInput");
        const customValueInput = document.getElementById(
          "customDeveloperCount"
        );
        const isCustomOption =
          element.closest(".btn-radio").id === "customDeveloperOption";
        if (isCustomOption) {
          customInput.style.display = "block";
          selectedDeveloperCount = null;
        } else {
          customInput.style.display = "none";
          customValueInput.value = "";
          selectedDeveloperCount = element
            .closest(".btn-radio")
            .querySelector("span")
            .textContent.trim();
          nextStep();
        }
      }

      function updateProgressBar() {
        const steps = document.querySelectorAll(".step");
        const totalSteps = steps.length;
        const percentage = Math.round((currentStep / (totalSteps - 1)) * 100);
        const radius = 15.5;
        const circumference = 2 * Math.PI * radius;
        const offset = circumference - (percentage / 100) * circumference;
        document
          .querySelectorAll(".progress-meter-progress")
          .forEach((circle) => {
            circle.style.strokeDasharray = `${circumference}px`;
            circle.style.strokeDashoffset = `${offset}px`;
          });

        document.querySelectorAll(".lp-completion-rate").forEach((el) => {
          el.textContent = `${percentage}%`;
        });
      }

      function showStep(index) {
        const steps = document.querySelectorAll(".step");
        steps.forEach((step, i) => {
          step.classList.toggle("active", i === index);
        });
        updateProgressBar();
        setTimeout(() => {
          $(".step.active .js-select2").select2({
            placeholder: "Select a country",
            allowClear: true,
            dropdownParent: $(".step.active"),
          });
        }, 50);
      }
      const customInputs = document.querySelectorAll(".customdata");

      customInputs.forEach((custominput) => {
        custominput.addEventListener("keydown", function (e) {
          if (e.key === "Enter" && this.value.trim() !== "") {
            selectedOption = this.value.trim();
            nextStep();
          }
        });
      });

      function nextStep() {
        const steps = document.querySelectorAll(".step");
        if (currentStep < steps.length - 1) {
          currentStep++;
          showStep(currentStep);
        }
        if (selectedOption === "Other (Custom Needs)") {
          const customText = document
            .getElementById("customNeedsText")
            .value.trim();
        }
        if (
          document.getElementById("customDeveloperCount") &&
          document.getElementById("customDeveloperInput").style.display ===
            "block"
        ) {
          selectedDeveloperCount = document
            .getElementById("customDeveloperCount")
            .value.trim();
        }
      }

      function prevStep() {
        if (currentStep > 0) {
          currentStep--;
          showStep(currentStep);
        }
      }

      document.addEventListener("DOMContentLoaded", () => {
        showStep(currentStep);
        toggleBudgetOptions();
      });

      function toggleBudgetOptions() {
        const billingType = document.querySelector(
          'input[name="billingType"]:checked'
        ).value;
        const hourlyOptions = document.getElementById("hourlyBudgetOptions");
        const monthlyOptions = document.getElementById("monthlyBudgetOptions");

        if (billingType === "hourly") {
          hourlyOptions.style.display = "block";
          monthlyOptions.style.display = "none";
        } else {
          hourlyOptions.style.display = "none";
          monthlyOptions.style.display = "block";
        }
      }
      function handleOptionSelect(type) {
        if (type === "hourly") {
          document.getElementById("hourlyCustomValue").value = "";
          document.getElementById("hourlyCustomInput").style.display = "none";
        } else if (type === "monthly") {
          document.getElementById("monthlyCustomValue").value = "";
          document.getElementById("monthlyCustomInput").style.display = "none";
        }
        nextStep();
      }

      function handleCustomSelect(type) {
        document.getElementById("hourlyCustomInput").style.display = "none";
        document.getElementById("monthlyCustomInput").style.display = "none";
        if (type === "hourly") {
          document.getElementById("hourlyCustomInput").style.display = "block";
        } else if (type === "monthly") {
          document.getElementById("monthlyCustomInput").style.display = "block";
        }
      }
      function validateName() {
        const nameInput = document.getElementById("fullName");
        const nameError = document.getElementById("nameError");
        const name = nameInput.value.trim();

        // Check if name has at least 2 words (first and last name)
        if (name === "" || name.split(" ").length < 2) {
          nameError.style.display = "block";
          nameInput.focus();
          return false;
        }

        nameError.style.display = "none";
        nextStep();
        return true;
      }

      function validateEmail() {
        const emailInput = document.getElementById("emailAddress");
        const emailError = document.getElementById("emailError");
        const email = emailInput.value.trim();

        // Simple email regex validation
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

        if (!emailRegex.test(email)) {
          emailError.style.display = "block";
          emailInput.focus();
          return false;
        }

        emailError.style.display = "none";
        nextStep(); // Proceed to next step if validation passes
        return true;
      }

      document.addEventListener("DOMContentLoaded", function () {
        window.intlTelInput(document.getElementById("country_code"), {
          initialCountry: "us",
          separateDialCode: true,
          utilsScript:
            "https://cdn.jsdelivr.net/npm/intl-tel-input@25.3.1/build/js/utils.js",
        });
      });
    </script>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
    <script
      src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.bundle.min.js"
      integrity="sha384-kenU1KFdBIe4zVF0s0G1M5b4hcpxyD9F7jL+jjXkk+Q2h455rYXK/7HAuoJl+0I4"
      crossorigin="anonymous"
    ></script>
    <script type="text/javascript">
      var Tawk_API = Tawk_API || {},
        Tawk_LoadStart = new Date();
      (function () {
        var s1 = document.createElement("script"),
          s0 = document.getElementsByTagName("script")[0];
        s1.async = true;
        s1.src = "https://embed.tawk.to/67f9552343ce3e190eb807f5/1ioiv94sd";
        s1.charset = "UTF-8";
        s1.setAttribute("crossorigin", "*");
        s0.parentNode.insertBefore(s1, s0);
      })();
    </script>
  </body>
</html>
